...
import RemindMeDropdown from "@/components/reminders/RemindMeDropdown";
        ...
        <div className="flex items-center gap-0.5">
          ...
          <div className="relative">
            <button
              ...
              className="p-1.5 rounded-small transition-colors hover:bg-background-100"
              style={{ color: sensitivityLabel ? "rgb(138 9 9)" : "rgb(155 155 155)" }}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {showLabelDropdown && (
              <div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-lg shadow-lg border border-neutral-200 py-1 z-50">
                {([
                  { value: "attorney-client-privilege", label: "Attorney-Client Privilege", color: "rgb(153 27 27)", bg: "rgb(254 226 226)" },
                  { value: "confidential", label: "Confidential", color: "rgb(154 52 18)", bg: "rgb(255 237 213)" },
                  { value: "work-product", label: "Work Product", color: "rgb(30 64 175)", bg: "rgb(219 234 254)" },
                ] as const).map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => {
                      const newLabel = sensitivityLabel === opt.value ? null : opt.value;
                      setSensitivityLabel(newLabel);
                      setShowLabelDropdown(false);
                      fetch("/api/mail/label", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ messageId: email.id, homeAccountId, label: newLabel }),
                      }).catch((err) => {
                        console.error("Failed to set label:", err);
                        setSensitivityLabel(sensitivityLabel);
                      });
                    }}
                    className="w-full px-3 py-2 text-left text-xs font-medium flex items-center gap-2 hover:bg-neutral-50 transition-colors"
                  >
                    <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: opt.color }} />
                    {opt.label}
                    {sensitivityLabel === opt.value && (
                      <svg className="w-3.5 h-3.5 ml-auto" style={{ color: "rgb(138 9 9)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                ...
                {sensitivityLabel && (
                  <>
                    ...
                    <button
                      onClick={() => {
                        setSensitivityLabel(null);
                        setShowLabelDropdown(false);
                        fetch("/api/mail/label", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({ messageId: email.id, homeAccountId, label: null }),
                        }).catch(console.error);
                      }}
                      className="w-full px-3 py-2 text-left text-xs font-medium text-neutral-500 hover:bg-neutral-50 transition-colors"
                    >
                      Remove Label
                    </button>
                  </>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto px-8 py-8">
          ...
          <div className="flex items-start gap-3 mb-6">
            <h1 className="text-2xl font-bold text-neutral-900 leading-tight">{email.subject}</h1>
            <div className="flex items-center gap-2 flex-shrink-0 mt-1">
              {isPinned && (
                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-sm flex-shrink-0" style={{ backgroundColor: "rgb(254 242 242)", color: "rgb(138 9 9)" }}>
                  Pinned
                </span>
              )}
              {sensitivityLabel && (() => {
                const cfg: Record<string, { text: string; bg: string; color: string }> = {
                ...
                const c = cfg[sensitivityLabel];
                return c ? (
                  <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-sm" style={{ backgroundColor: c.bg, color: c.color }}>
                    {c.text}
                  </span>
                ) : null;
              })()}
