        ...
        <div className="flex-shrink-0 flex items-center gap-2">
          {!account.isDefault && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-small border transition-colors disabled:opacity-60"
              style={{ color: "rgb(58 58 58)", borderColor: "rgb(229 229 229)", backgroundColor: "white" }}
              onMouseEnter={(e) => { if (!setDefaultLoading) e.currentTarget.style.backgroundColor = "rgb(245 245 245)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "white"; }}
            >
...
export default function AccountsClient({ accounts: initialAccounts }: { accounts: Account[] }) {
  ...
  const setDefaultInStore = useAccountStore((s) => s.setDefaultAccount);
  ...
  async function handleSetDefault(account: Account) {
    ...
    try {
      ...
      // Move the Default badge locally + keep the global store in sync
      setAccounts((prev) => prev.map((a) => ({ ...a, isDefault: a.id === account.id })));
      setDefaultInStore(account.homeAccountId);
    } catch (err) {
              ...
              <div className="space-y-3">
                {accounts.map((account) => (
                  <AccountCard
                    ...
                    setDefaultLoading={settingDefaultId === account.id}
                  />
                ))}
              </div>
            )}
